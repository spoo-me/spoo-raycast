...
export async function signIn(): Promise<DeviceTokenResponse> {
  ...
  // The PKCE client already sent request.codeChallenge (S256) at login;
  // redeem the code with the matching verifier the backend now requires.
  const tokens = await exchangeCode(apiBaseUrl, authorizationCode, request.codeVerifier);
